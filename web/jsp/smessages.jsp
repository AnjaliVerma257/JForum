...
<title>
   <%
    ...
    Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/jforum?","root","system");    
       ...
       ResultSet rs=stmt.executeQuery("select count(*) from messages where status='unread' and sender_id='"+uid+"'");
      
       int n=0;
       while(rs.next())
             n=rs.getInt(1);
       out.print("Sent Messages[");
            out.print(n);
            out.print("]");
            conn.close();
                    }catch(Exception e){}
...
ResultSet rs1;
String ab="select * from messages where sender_id=? order by msg_id limit ?,?";
...
int ide=rs1.getInt("reciever_id");
rs2=str.executeQuery("select name from userdetails where userid="+ide+"");
if(rs2.next())
send=rs2.getString(1);
out.println("<b style='font-style:italic; margin-left:270px;' >Sent To:  "+send+""+"&nbsp;&nbsp;&nbsp;&nbsp;On: " +date +"&nbsp;&nbsp  "+ time+"</b>");
...
for(i=1;i<=(nop+1);i++)
{
out.print(" <a href='smessages.jsp?mese="+i+"' onclick='smessages.jsp?mese="+i+"' name='a"+i+"'>");
out.print(i);
out.print("</a>");
}
out.print("</div>");
